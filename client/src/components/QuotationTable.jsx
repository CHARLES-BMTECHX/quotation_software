...
export default function QuotationTable() {
  ...
  const { data, isLoading, isFetching } = useQuery({
    queryKey: ["quotations", page, search, limit],
    queryFn: () => fetchQuotations({ page, limit, search }),
  ...
  // Fixed: quotations is already the array
      ...
      <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="flex items-center gap-4 w-full">
              <div className="relative flex-1 sm:w-64">
                <Search
                  className="absolute left-3 top-1/2 -translate-y-1/2 text-yellow-200"
                  size={18}
                />
                <input
                  type="text"
              ...
              <select
                ...
                className="px-3 py-2 bg-white/20 text-white border border-white/30 rounded-lg focus:ring-2 focus:ring-yellow-300 text-sm"
              >
                <option className="text-yellow-600" value={5}>
                  5 per page
                </option>
                <option className="text-yellow-600" value={10}>
                  10 per page
                </option>
                <option className="text-yellow-600" value={25}>
                  25 per page
                </option>
              </select>
            </div>
            ...
            <Link
              to="/create"
              className="bg-white text-yellow-600 px-5 py-2 rounded-lg font-semibold hover:bg-yellow-50 transition shadow-md"
            >
              + New Quotation
            </Link>
          </div>
        ...
        {quotations.length === 0 ? (
          <div className="text-center py-16 text-gray-500">
            <p className="text-xl font-medium">No quotations found</p>
            {search && (
          ...
          <>
            ...
            <div className="lg:hidden p-4 space-y-4">
              {quotations?.data?.map((q) => (
                <div
                  key={q._id}
                  className="bg-gradient-to-r from-yellow-50 to-white p-5 rounded-xl shadow-md border border-yellow-100"
                >
